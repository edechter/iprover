...
BASE_NAMES_BEFORE_LEXER = lib union_find options statistics bit_vec tableArray heap priority_queues tree trie trie_func vectorIndex abstDB abstAssignDB symbol symbolDB var term termDB orderings subst substBound dismatching clause selection clauseAssignDB clauseDB parser_types logic_interface 
...
BASE_NAMES_AFTER_LEXER = parser_tptp parseFiles splitting unif unifIndex discrTree subsetSubsume subsumptionIndex eq_axioms cMinisat propSolver prop_solver_exchange type_inf tstpProof bmc1Axioms inference_rules predElim model_inst finite_models preprocess prep_sem_filter prep_sem_filter_unif large_theories discount instantiation
