...
type parser_state =
    {
     symbol_db_ref       : SymbolDB.symbolDB ref;
     term_db_ref         : TermDB.termDB ref;
     mutable parsed_clauses  : clause list;
(* parsed clauses does not include negated_conjectures *)
     mutable neg_conjectures : clause list;
     mutable includes        : includes list;

(* auxilary fields used during parsing *)
(* max_var is used for renaming vars into iprover vars *)
     mutable max_var             : var;
...
let distinct : (((term list) list) ref)  = ref [] 
                             (* list of $distinct(t_1,..,t_n)  *)
                             (* represented as list of lists of terms *)
                             (* note distinct are not in added to the clauses! *)
...
let init () = 
(*  symbol_db_ref   := (SymbolDB.create_name "Symbols_DB");*)
 (* term_db_ref     := (TermDB.create_name "Terms_DB");*)
  parsed_clauses  := [];
  neg_conjectures := [];
  includes        := [];
  distinct        := [];
...
(*----------------parsing functions--------------*)
(* terms are put to TermDB during Clause.normalise *)
...
(* Can not use assign_type, since it is part of the key !*) 
(*
let plain_atomic_formula_fun term = 
(* twice as much memory /time if you use this line ... don't know why...*)
(* shift it to the end *)
...
(*
let equality_fun term1 term2 = 
 create_theory_term Symbol.symb_equality [term1;term2]
...
(* future: add negated_conjecture stuff after parsing *)
(* symbol types: Pred/Fun are needed for e.g. eq axioms ! *)
...
(*	   failwith 
	("parser_types, plain_term_fun_typed type of symbol "
...
let defined_pred_fun name args =
  ...
  |"=" -> 
    (* no all equalities are sorted and = is replaces by *)
    (* $equality_sorted(symb_default_type, args) *)
    (* create_theory_term Symbol.symb_equality args*)
      equality_fun args 
